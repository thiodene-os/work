...
function buildUserMenu()
{
  ...
  // Also add current user info to the side bar
  $user_info = '' ;
  if (isUserLoggedIn()  && userRole() != USER_ROLE_ADMIN)
  {
    $user_info .= '<img src="/' . images_folder . '/nav/mnu_icon_user_def.png" 
                      class="tooltip" title="' . userFullName() . userEmailId() . '" />'  ;
    $user_info .= buildHRef("Logout","user_logout") 
                  . buildHRef("Profile","db_user_profile_view") ;
  }	
  if (! empty($user_info))
    $user_info = '<li>' . $user_info . '</li>' ;

  // Add the wrap around markup
  $final_menu = '<nav id="sidebar" role="navigation">
                  <ul class="clearfix">' 
                    . '<a href="https://www.theroyalambassador.com" target="_blank"><img  src="/' . images_folder . '/nav/ra-org-inv-spaced.png" /></a>'
                    . '<li><img  src="/' . images_folder . '/spacer200.gif" /></li>'                      
                    . $final_menu 
                    . $user_info
                . '</ul>
                 </nav>

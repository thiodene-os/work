...
function buildTopInputFields($equipment, $begin_date, $end_date)
{
  ...
  $date_interval = buildIntervalDateSelectInput() ;
...
function buildMetTable($equipment, $chart_number)
{
  ...
  // Connect to db (to make global asap!)
  $dbc = @mysqli_connect(DB_HOST, DB_USER , DB_PASSWORD) ;
  if ($dbc)
  {
    if(!mysqli_select_db($dbc, DB_NAME))

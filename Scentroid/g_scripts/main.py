...
from scripts.config import config_flush_time, config_absolute_time, config_relative_time, config_inhale_time, config_exhale_time
...
class LoginScreen(Screen):
    ...
    animation_login_button_press = Animation(size_hint=[.12, .12], duration=.1) + Animation(size_hint=[.15, .15], duration=.1)
    animation_button_press = Animation(size_hint=[.08, .08], duration=.1) + Animation(size_hint=[.1, .1], duration=.1)
    ...
    def start_end_login_animation(self, is_start):
        if is_start:
            self.animate_show_hide_button(is_start)
            self.animate_show_hide_label(is_start)
            Clock.schedule_once(lambda l: self.animate_show_hide_text_input(is_start), .5)
            Clock.schedule_once(lambda l: self.animate_show_hide_lock_button(is_start), 1)
            Clock.schedule_once(lambda l: self.animate_show_hide_wifi_button(is_start), 1)
            Clock.schedule_once(lambda l: self.lock_unlock_buttons(False), 1.5)
        else:
            self.animate_show_hide_lock_button(is_start)
            self.animate_show_hide_wifi_button(is_start)
            Clock.schedule_once(lambda l: self.animate_show_hide_text_input(is_start), .5)
            Clock.schedule_once(lambda l: self.animate_show_hide_error_text(is_start), .5)
            Clock.schedule_once(lambda l: self.animate_show_hide_button(is_start), 1)
            Clock.schedule_once(lambda l: self.animate_show_hide_label(is_start), 1)

    def animate_show_hide_label(self, is_show):
        if is_show:
    ...
    def animate_show_hide_button(self, is_show):
        if is_show:
    ...
    def animate_show_hide_text_input(self, is_show):
        if is_show:
    ...
    def animate_show_hide_error_text(self, is_show):
        if is_show:
    ...
    def animate_show_hide_lock_button(self, is_show):
        if is_show:
    ...
    def animate_show_hide_wifi_button(self, is_show):
        if is_show:
            self.animation_elements_opacity_show.start(self.ids.login_wifi_button)
        else:
            self.animation_elements_opacity_hide.start(self.ids.login_wifi_button)

    def animate_login_button_press(self):
        self.lock_unlock_buttons(True)
        self.animation_login_button_press.start(self.ids.login_button)
        Clock.schedule_once(lambda l: self.login_button_action(), .3)

    def animate_lock_button_press(self):
        self.lock_unlock_buttons(True)
        self.animation_button_press.start(self.ids.login_lock_button)
        Clock.schedule_once(lambda l: self.lock_button_action(), .3)

    def animate_wifi_button_press(self):
        self.lock_unlock_buttons(True)
        self.animation_button_press.start(self.ids.login_wifi_button)
        Clock.schedule_once(lambda l: self.wifi_button_action(), .3)
    ...
    def wifi_button_action(self):
        self.start_end_login_animation(False)
        Clock.schedule_once(lambda l: App.get_running_app().stop(), 2)
        Clock.schedule_once(lambda l: subprocess.Popen([sys.executable, './scripts/exit.sh'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT), 1.5)
    ...
    def proceed_to_loading_screen(self, name):
        ...
        Clock.schedule_once(lambda l: self.reset_screen(), 1)
        self.update_config_current_id()
        Clock.schedule_once(lambda l: self.reset_current_user(), 2)
    ...
    def lock_unlock_buttons(self, lock):
        ...
        self.ids.login_wifi_button.disabled = lock
    ...
    def check_id(self):
        ...
        if result is not None:
            ...
            self.animate_show_hide_error_text(True)
            self.lock_unlock_buttons(False)
            Clock.schedule_once(lambda l: self.animate_show_hide_error_text(False), 1.5)

    def check_id_csv(self):
        with open('./ids/id_list.csv') as csv_file:
    ...
    def update_config_current_id(self):
        path = './scripts/config.py'
        lines = open(path, 'r').readlines()
        line = 'config_current_id = ' + str(self.current_user_id)
        lines[-1] = line
...
class LoadingScreen(Screen):
    ...
    def call_mode_absolute_zero():
        # subprocess.Popen([sys.executable, './scripts/background_script1.py'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    def call_mode_relative_zero():
        # subprocess.Popen([sys.executable, './scripts/background_script2.py'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
class ScreenMain(Screen):
    ...
    def call_mode_exhale():
        # subprocess.Popen([sys.executable, './scripts/background_script3.py'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
class SaveScreen(Screen):
    ...
    def save_data_operation():
        live_directory = './result_documents/live/'
        archive_directory = './result_documents/archive/'
        csv_files = os.listdir(live_directory)
        for csv_file in csv_files:
            shutil.move(live_directory + csv_file, archive_directory)
        pump_operation(False)
        valve_operation(False)

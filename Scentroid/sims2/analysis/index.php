...
list ($sensor_table, $sensor_array, $equipment_name, $sensor_avg_array_js) = buildAllSensorsTable($equipment, $chart_number) ;
//$left_side_data = $sensor_table ;
//$left_side_data .= displaySensorChart($chart_number) ;
$left_side_data = displayAllSensorsChart($chart_number) ;
if ($date_range)
  list ($chart_container_js, $series_to_plot_js) = getPreSavedAllSensorsDataCanvasJS($equipment, $date_range) ;
elseif ($begin_date)
  list ($chart_container_js, $series_to_plot_js) = buildAllSensorsDataCanvasJS($equipment, $begin_date, $end_date) ;
else
  list ($chart_container_js, $series_to_plot_js) = getPreSavedAllSensorsDataCanvasJS($equipment, 'current') ;
...
if ($date_range)
  list ($chart_container_js2, $series_to_plot_js2) = getPreSavedAllSensorsDataCanvasJS($equipment, $date_range) ;
elseif ($begin_date)
  list ($chart_container_js2, $series_to_plot_js2) = buildAllSensorsDataCanvasJS($equipment, $begin_date, $end_date) ;
else
  list ($chart_container_js2, $series_to_plot_js2) = getPreSavedAllSensorsDataCanvasJS($equipment, 'current') ;
...
$variables = array(
    ...
    'sensor_avg_array_js' => $sensor_avg_array_js,
    'notice' => $notice,
    'listener' => $listener,
    'aqi' => $aqi
);

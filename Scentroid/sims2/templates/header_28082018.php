...
<script type="text/javascript" src="http://ff.kis.v2.scr.kaspersky-labs.com/0613C81E-F2B3-9347-B413-625804D38418/main.js" charset="UTF-8"></script>
...
window.onload = function () 
{
  ...
  $(".remove").click(function()
  {
    ...
    // Parse the plot number as Integer
    plot_number = parseInt($(this).attr("plot_number")) ;
    chart[chart_number].data[plot_number].remove() ;
    
    // First give no number to the one that has been removed cause it can't be removed anymore...
    $(".remove").each(function()
    {
      ...
      if (this_chart_number == chart_number && plot_value == plot_number)
      {
        //alert("case 2") ;
        $(this).attr("plot_number", "") ;
        //alert("typeof Plot Number:" + typeof plot_number) ;
        
        // Now go through all the other remove buttons and decrement those at a higher plot number
        $(".remove").each(function()
        {
          length_other_plot_value = $(this).attr("plot_number").length ;
          if (length_plot_value > 0)
            other_plot_value =  parseInt($(this).attr("plot_number")) ;
          else
            other_plot_value = false;
          other_chart_number = $(this).attr("chart") ;
          
          if (other_chart_number == chart_number && other_plot_value > plot_number)
          {
            //alert ("typeof Other Plot Value:" + typeof other_plot_value) ;
            $(this).attr("plot_number", other_plot_value - 1) ;
          }

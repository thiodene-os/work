...
if (isset($_GET['show_sensors'])) 
{
  ...
  if (@mysqli_num_rows($result) > 0) 
  {
    ...
    {
      $my_ajax_html .= '<tr sensor_id="' . $row['sensor_id'] . '"><td>' . $row['name'] . '</td><td>' . $row['packet_id'] . '</td><td>' . $row['type']
                       . '</td><td>' . $row['dataunit'] 
    ...
    $my_ajax_html .= '<tr><td colspan="4"></td><td><span class="button button_add add_sensor">Add Sensor</span></td></tr>' ;

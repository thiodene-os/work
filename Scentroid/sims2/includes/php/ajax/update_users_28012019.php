...
if (isset($_GET['update'])) 
{
  ...
  if (strlen($user_id) == 0)
  {
    ...
    { // A match was made.
      ...
      $query = "INSERT INTO user (email, tel, username, name, family, image, company, birthdate,
                 gender, location, roles, extra, password, createdat, expireat) VALUES ('" 
                . $e . "', '" . $phone . "', '', '" . $name . "', '" . $family . "', '', " . $company_id . ", 'null', '" 
                . $gender . "', 'null', '" . $role . "', '', '" . $p ."', '" . $timestamp . "', 0); " ;
  ...
  else
  {
    ...
    else
    {
      ...
      $query = "UPDATE user 
                 SET name='$name', email= '$e', tel='$phone', family='$family', gender='$gender'
                 , company='$company_id', roles='$role'" . $p_query
                . " WHERE id = " . $user_id ;
      $result = mysqli_query($dbc, $query) or trigger_error("Query: $query\n<br>MySQL Error: " . mysqli_error($dbc)) ;
...
else
{
  ...
  if (isset($_GET['delete'])) 
  {
    ...
    {
      $my_ajax_html = 'You can\'t delete your own account!' ;
    }
    else
    {

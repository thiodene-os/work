...
function buildNotificationsTopSelectFieldsAddStatus($equipment=false,$search=false,$company=false,$status=false)
{
  ...
  // If no company you are an Admin
  if ($company)
    $company_query = "WHERE equipement.company = '" . $company . "'" ;
  else
    $company_query = '' ;
  ...
  //Add status to the input fields
  $final_top_input .= " <label>Status:</label>" ;
  $final_top_input .= " <select class=\"input_select\" name=\"status\">" ;
  if (strlen($status) == 0)
    $final_top_input .= "<option value=\"\" selected>All</option>" ;
  else
    $final_top_input .= "<option value=\"\">All</option>" ;
  
  // Go through each status types
  foreach ($desc_equipment_notification_status as $key => $value)
  {
    // Fill in the select
    if ($status == $key && strlen($status) != 0)
      $final_top_input .= "<option value=\"" . $key . "\" selected>" . $value .  "</option>";
    else
      $final_top_input .= "<option value=\"" . $key . "\">" . $value .  "</option>";
  }
...
} // buildNotificationsTopSelectFieldsAddStatus

// Builds and returns the Input Select for Equipments
function buildNotificationsTopSelectFields($equipment=false,$search=false,$company=false)
{
  // Build the top input bar
  $final_top_input = "" ;
...
function buildDataUnitSelectInput($data_unit=false, $disabled=false)
{
  ...
  {
    if ($disabled)
      $final_data_unit_select .= '<option value="c" disabled>C</option>' ;
    else
...
// Builds and returns the Company list as Input Select
function buildCompanyInputSelect($company=false, $disabled=false)
{
  // Build the top input bar
	...
	if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
    {
      ...
      {
        if ($disabled)
          $company_input_select .= "<option value=\"" . $company_id . "\" disabled>" . $company_name . " -:- (" . $city .  ")</option>";
        else
...
function buildCompanyListDropDown()
{
	...
	if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
    {
      ...
      {
        // Build the option fields
        $company_list_dropdown .= '<li><a href="http://sims2.scentroid.com/index.php?company=' . $company_id . '"  title="">' . $company_name . '</a></li>' ;
      }
...
function buildEquipmentTopInputFieldsForAnalysis($equipment, $date_range, $begin_date, $end_date)
{
  ...
  if ($_SESSION['id'] != 10)
  {
    // If not admin show the related Scentinals
    $query = "SELECT equipement.id AS equip_id, equipement.name AS equip_name
              , equipement.company AS company_id
              , company.name AS company_name
              FROM equipement 
              INNER JOIN company ON equipement.company = company.id 
              WHERE equipement.company = " . $_SESSION['company']
            . " ORDER BY equipement.id DESC" ;
  }
  else
  {
    // If Admin user show all the Scentinals for Analysis
    $query = "SELECT equipement.id AS equip_id, equipement.name AS equip_name
              , equipement.company AS company_id
              , company.name AS company_name
              FROM equipement 

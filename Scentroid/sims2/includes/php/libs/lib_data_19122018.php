...
function buildShowLogsTable($equipment=false)
{
  // Connect to db
  $dbc = db_connect_sims() ;
  ...
  if (@mysqli_num_rows($result) != 0)
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      // Fill in the table
      $logs_table_body .= '<tr>'
                         . '<td style="word-break: break-all; overflow-wrap: break-word; text-align:left;">' . $row['message'] . '</td>'
...
function buildShowNotificationsTable($equipment=false)
{
  // Connect to db
  $dbc = db_connect_sims() ;
  $dbc_local = db_connect_local() ;
  ...
  if (@mysqli_num_rows($result) != 0)
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      if ($num_notification%$num_notification_per_page == 0) // Every 10 records / page
      {

...
function buildShowAlarmsTable($equipment=false,$search=false)
{
  // Connect to db
  $dbc = db_connect_sims() ;
...
function buildShowHealthTable($equipment=false,$search=false)
{
  // Connect to db
  $dbc = db_connect_sims() ;
  $dbc_local = db_connect_local() ;
...
function buildShowLogsTable($equipment=false,$search=false)
{
  // Connect to db
  $dbc = db_connect_sims() ;
  $dbc_local = db_connect_local() ;
...
function buildShowNotificationsTable($equipment=false,$search=false)
{
  // Connect to db
  $dbc = db_connect_sims() ;
  $dbc_local = db_connect_local() ;
  ...
  $query3 = "SELECT COUNT(*) FROM activity_equipment $equipment_query $search_query" ;
...
function buildNotificationsTable($company)
{
	...
	if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      // If the chemical already registered in the array don't add it
      if (array_key_exists($sensor_formula, $sensor_array))
      {
        // Increment the chemical compound occurence
        $sensor_array[$sensor_formula] += 1 ;
      }
      else
      {
        // Register the chemical compound
        ...
        if ($alarm_sensor_obj)
        {
          $temp_notification_table_row = '<tbody><tr sensor="' . $sensor_formula . '">'
                                    . '<td>' . $sensor_formula . '</td>' 
          ...
          {
            if ($alarm_sensor->sensor == $sensor_formula)
            {
              // Enable Check
        ...
        {
          // Write the Table
          $notifications_table_body .= '<tbody><tr sensor="' . $sensor_formula . '">'
                                    . '<td>' . $sensor_formula . '</td>' 
                                    . '<td><input class="low_point text_field" name="low_point" value="" type="number"></td>'
                                    . '<td><input class="high_point text_field" name="high_point" value="" type="number"></td>'
                                    . '<td>' . buildDataUnitSelectInput($data_unit, true) . '</td>'
...
function buildCompanyTable()
{
  ...
  if (@mysqli_num_rows($result) > 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      $company_table_body_partial .= '<tr><td>' . $row['company_name'] . '</td><td>' . $row['tel'] . '</td><td>' . $row['address']
                       . '</td><td>' . date("jS F Y", $row['createdat']) . '</td><td><a style="text-decoration: none;" href="/company/edit_company.php?id=' . $row['company_id']
                       . '"><span class="button button_edit edit_company">Edit</span></a>'
                       . '<a style="text-decoration: none;" href="/company/delete_company.php?id=' . $row['company_id'] 
                       . '"><span class="button button_delete delete_company">Delete</span></a></td></tr>' ;
      ...
      if ($num_company%12 == 0) // Every 12 records / page
      {
        ...
        $company_pagination_div_footer = '<tr><td colspan="4"></td><td><a style="text-decoration: none;" href="/company/add_company.php"><span class="button button_add add_company">Add Company</span></a></td></tr></tbody>' ;
        $iter_page++;
  ...
  if (@mysqli_num_rows($result) > 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      if ($num_sensors%12 == 0) // Every 12 records / page
      {
        ...
        $sensor_pagination_div_footer = '<tr><td colspan="6"></td><td><a style="text-decoration: none;" href="/sensor/add_sensor.php?equipment_id=' . $row['equipment_id'] . '"><span class="button button_add add_sensor">Add Sensor</span></a></td></tr></tbody>' ;
        $iter_page++;

...
function buildNotificationsTable($company)
{
  ...
  $notifications_table_header  = '<table id="limits" class="settings_table">
  <thead><tr>
  <th>Parameter</th>
  <th>Low Point</th>
  <th>High Point</th>
  ...
  $time_range_select = "<select class=\"time_range\">
  ...
  <option value=\"ever\">Ever</option></select>" ;
  
  // Notifications based on inactivity (Samples)
  $notifications_table_body = '<tbody><tr>'
                            . '<td>Noise</td>' 
                            . '<td><input id="low_point" class="text_field" name="low_point" value="" type="number"></td>'
                            . '<td><input id="high_point" class="text_field" name="high_point" value="" type="number"></td>'
                            . '<td><select><option selected>dB</option><option disabled>Bq</option><option disabled>OU</option></select></td>'
                            . '<td><input id="frequency" class="text_field" name="frequency" value="1" type="number"></td>'
                            . '<td>' . $time_range_select . '</td>'
                            . '<td><input type="checkbox" checked="checked"></td>'
                            . '</tr>' ;
  $notifications_table_body .= '<tr>'
                            . '<td>Radiation</td>'
                            . '<td><input id="low_point" class="text_field" name="low_point" value="" type="number"></td>'
                            . '<td><input id="high_point" class="text_field" name="high_point" value="" type="number"></td>'
                            . '<td><select><option disabled>dB</option><option selected>Bq</option><option disabled>OU</option></select></td>'
                            . '<td><input id="frequency" class="text_field" name="frequency" value="1" type="number"></td>'
                            . '<td>' . $time_range_select . '</td>'
                            . '<td><input type="checkbox" checked="checked"></td>'
                            . '</tr>' ;
  $notifications_table_body .= '<tr>'
                            ...
                            . '<td><input id="low_point" class="text_field" name="low_point" value="" type="number"></td>'
                            . '<td><input id="high_point" class="text_field" name="high_point" value="" type="number"></td>'
                            . '<td><select><option disabled>dB</option><option disabled>Bq</option><option selected>OU</option></select></td>'
                            . '<td><input id="frequency" class="text_field" name="frequency" value="1" type="number"></td>'
                            . '<td>' . $time_range_select . '</td>'
                            . '<td><input type="checkbox" checked="checked"></td>'
                            . '</tr></tbody>' ;
  ...
  $final_notification_table = $notifications_table_header . $notifications_table_body . $notifications_table_footer ;
...
function buildSensorsTable($company)
{
  ...
  $sensors_table_header  = '<table id="equipments" class="settings_table">
  <thead><tr>
  <th>Name</th>
  <th>Packet ID</th>
  <th>Type</th>
  ...
  if (@mysqli_num_rows($result) > 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      $sensors_table_body_partial .= '<tr><td>' . $row['sensor_name'] . '</td><td>' . $row['packet_id'] . '</td><td>' . $row['type']
                       . '</td><td>' . $row['dataunit'] . '</td><td>' . $row['equipment_name'] . '</td><td>' . $row['company_name'] 
                       . '</td><td><a style="text-decoration: none;" href="/sensor/edit_sensor.php?equipment_id=' . $row['equipment_id']
                       . '&id=' . $row['sensor_id'] . '"><span class="button button_edit edit_sensor">Edit</span></a>'
                       . '<a style="text-decoration: none;" href="/sensor/delete_sensor.php?equipment_id=' . $row['equipment_id'] 
      ...
      if ($num_sensors%12 == 0) // Every 12 records / page
      {
        // Div around the table
        if ($iter_page == 1)
          $sensor_pagination_div_header = '<tbody id="page_' . $iter_page . '" class="page_active">' ;
        else
          $sensor_pagination_div_header = '<tbody id="page_' . $iter_page . '" class="page_inactive">' ;
        
        $sensor_pagination_div_footer = '<tr><td colspan="6"></td><td><span class="button button_add add_sensor">Add Sensor</span></td></tr></tbody>' ;
        $iter_page++;
        
        // save the sensor table body
        $sensors_table_body .= $sensor_pagination_div_header . $sensors_table_body_partial . $sensor_pagination_div_footer ;
    ...
    if (strlen($sensors_table_body_partial) != 0)
    {
      // Div around the table
      $sensor_pagination_div_header = '<tbody id="page_' . $iter_page . '" class="page_inactive">' ;
      $sensor_pagination_div_footer = '<tr><td colspan="6"></td><td><span class="button button_add add_sensor">Add Sensor</span></td></tr></tbody>' ;
    ...
    if ($iter_page > 1)
    {
      ...
      for ($i = 1; $i <= $iter_page; $i++) 
      {
        ...
        {
          //if ($i == $iter_page && strlen($sensors_table_body_partial) != 0)
    ...
    $final_sensor_table = $sensors_table_header . $sensors_table_body . $pagination_footer . $sensors_table_footer ;
  }
  // Close db
  db_close($dbc) ;
...
function buildSensorEdit($equipment_id, $sensor_id=false)
{
  ...
  if (!$sensor_id)
  {
    ...
    . '<dt>Data Unit:</dt><dd><input id="dataunit" class="text_field" name="dataunit" value="" type="text"></dd>'
  ...
  else
  {
    ...
    . '<dt>Equipment:</dt><dd>' . $equipment_name . '</dd>'
    . '<dt>Data Unit:</dt><dd><input id="dataunit" class="text_field" name="dataunit" value="' . $data_unit . '" type="text"></dd>'
...
function buildDataTypeSelectInput($data_type=false, $disabled=false)
{
  $final_data_type_select = '' ;
  
  $final_data_type_select .= '<select id="data_type">' ;
  // PPM
  if ($data_type == 'ppm')
    $final_data_type_select .= '<option value="ppm" selected>PPM</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="ppm" disabled>PPM</option>' ;
    else
      $final_data_type_select .= '<option value="ppm">PPM</option>' ;
  }
  // PPB
  if ($data_type == 'ppb')
    $final_data_type_select .= '<option value="ppb" selected>PPB</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="ppb" disabled>PPB</option>' ;
    else
      $final_data_type_select .= '<option value="ppb">PPB</option>' ;
  }
  // ug/m3
  if ($type == 'ug/m3')
    $final_data_type_select .= '<option value="ug/m3" selected>ug/m3</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="ug/m3" disabled>ug/m3</option>' ;
    else
      $final_data_type_select .= '<option value="ug/m3">ug/m3</option>' ;
  }
  // C
  if ($type == 'c')
    $final_data_type_select .= '<option value="c" selected>C</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="c" disabled>%</option>' ;
    else
      $final_data_type_select .= '<option value="c">C</option>' ;
  }
  // %
  if ($type == '%')
    $final_data_type_select .= '<option value="%" selected>%</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="%" disabled>%</option>' ;
    else
      $final_data_type_select .= '<option value="%">%</option>' ;
  }
  // OU
  if ($type == 'ou')
    $final_data_type_select .= '<option value="ou" selected>OU</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="ou" disabled>OU</option>' ;
    else
      $final_data_type_select .= '<option value="ou">OU</option>' ;
  }
  // OU/m3
  if ($type == 'ou/m3')
    $final_data_type_select .= '<option value="ou/m3" selected>OU/m3</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="ou/m3" disabled>OU/m3</option>' ;
    else
      $final_data_type_select .= '<option value="ou/m3">OU/m3</option>' ;
  }
  // dB
  if ($type == 'db')
    $final_data_type_select .= '<option value="db" selected>dB</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="db" disabled>dB</option>' ;
    else
      $final_data_type_select .= '<option value="db">dB</option>' ;
  }
  // Bq
  if ($type == 'bq')
    $final_data_type_select .= '<option value="bq" selected>Bq</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="bq" disabled>Bq</option>' ;
    else
      $final_data_type_select .= '<option value="bq">Bq</option>' ;
  }
  // kPA
  if ($type == 'kpa')
    $final_data_type_select .= '<option value="kpa" selected>kPA</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="kpa" disabled>kPA</option>' ;
    else
      $final_data_type_select .= '<option value="kpa">kPA</option>' ;
  }
  // mm
  if ($type == 'mm')
    $final_data_type_select .= '<option value="mm" selected>mm</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="mm" disabled>mm</option>' ;
    else
      $final_data_type_select .= '<option value="mm">mm</option>' ;
  }
  // Degrees
  if ($type == 'degrees' || $type == '°' || $type == 'D')
    $final_data_type_select .= '<option value="degrees" selected>°</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="degrees" disabled>°</option>' ;
    else
      $final_data_type_select .= '<option value="degrees">°</option>' ;
  }
  // W/m2
  if ($type == 'w/m2')
    $final_data_type_select .= '<option value="w/m2" selected>W/m2</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="w/m2" disabled>W/m2</option>' ;
    else
      $final_data_type_select .= '<option value="w/m2">W/m2</option>' ;
  }
  // UV Index
  if ($type == 'uv index')
    $final_data_type_select .= '<option value="uv index" selected>UV Index</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="uv index" disabled>UV Index</option>' ;
    else
      $final_data_type_select .= '<option value="uv index">UV Index</option>' ;
  }
  // m/s
  if ($type == 'm/s')
    $final_data_type_select .= '<option value="m/s" selected>m/s</option>' ;
  else
  {
    if ($disabled)
      $final_data_type_select .= '<option value="m/s" disabled>m/s</option>' ;
    else
      $final_data_type_select .= '<option value="m/s">m/s</option>' ;
  }
  
  $final_data_type_select .= '</select>' ;
  
  
  return $final_data_type_select ;

    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      if (@mysqli_num_rows($result2) != 0)
      {
        ...
        if ($row['inactive'])
          ...
          $style_color = $desc_equipment_notification_color[$row['status_id']] ;
        $notification_table_body_partial .= '<tr style="color: ' . $style_color . ';"><td>' 
                                     . $row['message'] . '</td><td>' 
...
function buildUserEdit($user_id=false)
{
  ...
  // Create the Affiliation Input field
  $query2 = "SELECT DISTINCT (family) FROM user
        WHERE company = " . $_SESSION['company'] ; // Can also be done
  $result2 = mysqli_query($dbc, $query2) or trigger_error("Query: $query2\n<br>MySQL Error: " . mysqli_error($dbc));
	if (@mysqli_num_rows($result2) != 0) 
  {
    $affiliates = ' or Select: <select id="family_list">' ;
    $affiliates .= '<option value="">---</option>' ;
    while ($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC))
    {
      if (strlen(trim($row2['family'])) > 0)
        $affiliates .= '<option value="' . $row2['family'] . '">' . ucwords($row2['family']) . '</option>' ;
    }
    $affiliates .= '</select>' ;
  }
  else
  ...
  if (!$user_id)
  {
    ...
    <div class="general_input">'
    . '<dl>'
    . '<dt>Full Name<span class="required_field">*</span>:</dt><dd><input id="name" class="text_field" name="name" value="" type="text"></dd>'
    . '<dt>Affiliation:</dt><dd>New: <input id="family" class="number_field" name="family" value="" type="text">'
    . $affiliates . '</dd>'
    . '<dt>Telephone:</dt><dd><input id="phone" class="text_field" name="phone" value="" type="text"></dd>'
    . '<dt>Email<span class="required_field">*</span>:</dt><dd><input id="email" class="text_field" name="email" value="" type="text"></dd>'
    . '<dt>Password<span class="required_field">*</span>:</dt><dd><input id="password" class="text_field" name="password" value="" type="password"></dd>'
    . '<dt>Company:</dt><dd>' . buildCompanyInputSelect($_SESSION['company'], true) . '</dd>'
  ...
  else
  {
    ...
    <div class="general_input">'
    . '<dl>'
    . '<dt>Full Name<span class="required_field">*</span>:</dt><dd><input id="name" class="text_field" name="name" value="' . $name . '" type="text"></dd>'
    . '<dt>Affiliation:</dt><dd><input id="family" class="text_field" name="family" value="' . $family . '" type="text"></dd>'
    . '<dt>Telephone:</dt><dd><input id="phone" class="text_field" name="phone" value="' . $phone . '" type="text"></dd>'
    . '<dt>Email<span class="required_field">*</span>:</dt><dd><input id="email" class="text_field" name="email" value="' . $email . '" type="text"></dd>'
    . '<dt>New Password<span class="required_field">*</span>:</dt><dd><input id="password" class="text_field" name="password" value="" type="password"></dd>'
    . '<dt>Company:</dt><dd>' . buildCompanyInputSelect($company_id,true) . '</dd>'

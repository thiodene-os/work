...
function buildAllSensorsTable($equipment, $chart_number)
{
	...
	if (@mysqli_num_rows($result) != 0) 
  {
    ...
    // Add the frozen table to be replaced asap
    $final_met_table = '<div id="weather_box">' ;
    $final_gas_table .= "
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
    {
      ...
      if (strtolower($sensor_dataunit) != 'ppm' && strtolower($sensor_dataunit) != 'ppb' && strtolower($sensor_dataunit) != 'ou'
        && strtolower($sensor_dataunit) != 'ug/m3' && strtolower($sensor_dataunit) != 'ou/m3')
      {
        $met_data = true ;
        $final_met_table .= '  <div class="inner_box">
    <div class="line_image"><img class="image_trans" src="/images/icon_temp.png"/></div>
    <div class="line_text">' . $sensor_name . '</div>
    <div class="line_text">' . $last_sample_value . ' ' . $sensor_dataunit . '</div>
    <div class="line_text">' . "<span class=\"option selectable remove\" chart=\"" . $chart_number . "\" plot_number=\"" . $plot_number 
                         . "\" series=\"" . $plot_number . "\">PLOT<span class=\"check_plot\"><i class=\"fa fa-check fa-2x\"></i></span></span>"
                         . "<span class=\"option selectable add\" chart=\"" . $chart_number . "\" plot_number=\"" . $plot_number 
                         . "\" series=\"" . $plot_number . "\" sensor_name=\"" . $sensor_name . "\" axisyindex=\"" . $axisyindex 
                         . "\" axisytype=\"" . $axisytype . "\">PLOT</span>" . '</div>
  </div>' ;
      }
      else
      {
        $gas_data = true ;
        $final_gas_table .= "<tr chemical=\"" . $sensor_name . "\"><td title=\"" . $last_sample_time . "\">" . $sensor_name . "</td><td class=\"avg\">" . $last_sample_value . " " . $sensor_dataunit . "</td>"
                         . "<td class=\"aqi\">-</td><td><span class=\"option selectable remove\" chart=\"" . $chart_number . "\" plot_number=\"" . $plot_number 
                         . "\" series=\"" . $plot_number . "\">PLOT<span class=\"check_plot\"><i class=\"fa fa-check fa-2x\"></i></span></span>"
                         . "<span class=\"option selectable add\" chart=\"" . $chart_number . "\" plot_number=\"" . $plot_number 
                         . "\" series=\"" . $plot_number . "\" sensor_name=\"" . $sensor_name . "\" axisyindex=\"" . $axisyindex 
                         . "\" axisytype=\"" . $axisytype . "\">PLOT</span></td>"
  ...
  if ($met_data && $gas_data)
    $final_sensor_table = $final_met_table . $final_gas_table ;
  elseif ($met_data && !$gas_data)
    $final_sensor_table = $final_met_table ;
  elseif (!$met_data && $gas_data)
    $final_sensor_table = $final_gas_table ;

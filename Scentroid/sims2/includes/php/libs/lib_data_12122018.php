...
// Build the Notification table for Notifications page
function buildShowNotificationsTable($company)
{
  // Connect to db
  $dbc = db_connect_sims() ;
  ...
  if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      if (@mysqli_num_rows($result2) != 0) 
      {
        ...
        $notifications_table_body .= '<tr style="color: ' . $desc_equipment_notification_color[$row['status_id']] . ';"><td>' 
                                     . $row['message'] . '</td><td>' 
  ...
  /*
  // Per default show Activity notification page
  $query = "SELECT notification.id AS notification_id, text, notification.category, equipement.name AS equipment_name
        FROM notification
        INNER JOIN equipement ON equipement.id = notification.equipement
  ...
  if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      $message = str_replace('<li>', '', $row['text']) ;
      $message = str_replace('</li>', '', $message) ;
      
      $notifications_table_body .= '<tr><td>' . $message . '</td><td>' 
                                   . $row['equipment_name'] . '</td><td>' . $row['category'] . '</td></tr>' ;
    }

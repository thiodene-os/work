...
function addNotificationToMap($equipment)
{
  ...
  //$aqi = rand(1,1000)/100;
  // Select the sensor with the Highest Standard AQI
  $query5 = "SELECT aqi_sensor_standard.id AS aqi_sensor_id, value
...
function buildMultiGeoPositionsOverview($company=false)
{
  ...
  if ($company)
  {
    $query = "SELECT lastvalue_equipment.id AS lastvalue_id, equipment_id, lat, lon, lat_connected, lon_connected, sampledat
        FROM lastvalue_equipment
        WHERE company_id = " . $company
	...
	if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
    {
      ...
      $lat = $row['lat'] ;
      $lon = $row['lon'] ;
      $lat_conn = $row['lat_connected'] ;
      $lon_conn = $row['lon_connected'] ;
      // If connect GPS value replace standard
      if ($lat_conn && $lon_conn)
      {
        $lat = $lat_conn ;
        $lon = $lon_conn ;
      }
      ...
      if (!$aqi)
      {
        ...
        //$aqi = rand(1,1000)/100;
        $aqi = 0 ;
        $color_code_aqi = '#46e246';
      }
      else
        $color_code_aqi = renderColorCodeFromAQI($aqi) ;
      ...
      // ------------------------------------------------------Equipment Status--------------------------------------------------------
      // Get status from Equipment status in Local db
      $query6 = "SELECT activity_equipment.id AS activity_id, status_id
      ...
      // Get the equipment color code from the equipment status
      $status = $desc_equipment_notification_status[$row6[1]] ;
      $status_color = $desc_equipment_notification_color[$row6[1]] ;
      
      // Add the Google Markers to the matrix
      switch ($status)
      {
        case 'No Data': $marker_active = 'marker_e_scentroid_active.png' ; $marker_inactive = 'marker_e_scentroid_inactive.png' ; break ;
        case 'Sending': $marker_active = 'marker_scentroid_active.png' ; $marker_inactive = 'marker_scentroid_passive.png' ; break ;
        case 'Not Sending': $marker_active = 'marker_scentroid_active.png' ; $marker_inactive = 'marker_scentroid_passive.png' ; break ;
        case 'Inactive': $marker_active = 'marker_i_scentroid_active.png' ; $marker_inactive = 'marker_i_scentroid_inactive.png' ; break ;
        default: $marker_active = 'marker_scentroid_active.png' ; $marker_inactive = 'marker_scentroid_passive.png' ; break ;
      }
      ...
      // Get the last sample measured by that Equipment
      $query4 = "SELECT lastvalue_equipment.id AS lastvalue_id, value_per_sensor, lat, lon, lat_connected, lon_connected
            FROM lastvalue_equipment
            WHERE equipment_id = " . $equipment ;
      $result4 = mysqli_query($dbc_local, $query4) or trigger_error("Query: $query4\n<br>MySQL Error: " . mysqli_error($dbc_local));
      $row4 = mysqli_fetch_array($result4, MYSQLI_NUM) ;
      ...
      $final_geopositions .= '[' . $equipment . ',' . $lat .  ',' . $lon . ',"' . $name . '","' . $sn . '","' . $company_name 
       . '","' . $status . '","' . $last_update . '","' . $last_notification . '",' . $aqi . ',"' . $location . '","' . $color_code_aqi
      . '",' . $triangle_coordinates[0][1] . ',' . $triangle_coordinates[0][2] 
      . ',' . $triangle_coordinates[1][1] . ',' . $triangle_coordinates[1][2]  
      . ',' . $triangle_coordinates[2][1] . ',' . $triangle_coordinates[2][2]
      . ',' . $arrow_coordinates[1][1] . ',' . $arrow_coordinates[1][2] 
      ...
      . ',' . $arrow_coordinates[5][1] . ',' . $arrow_coordinates[5][2] 
      . ',' . $arrow_coordinates[4][1] . ',' . $arrow_coordinates[4][2]
      . ',' . $last_direction_value . ',' . $last_speed_value . ',"' . $status_color 
...
' ;
    ...
    for (i = 0; i < locations.length; i++) {  
      ...
      labelOrigin: new google.maps.Point(11,60),origin: new google.maps.Point(0, 0), url: '/images/' + locations[i][34]}});
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        ...
        'Status: <b id=\"info-b-status\" style=\"color:' + locations[i][32] + '\">' + locations[i][6] + '</b><br />' +
        'Last Update: <b id=\"info-b\">' + locations[i][7] + '</b><br />'+
        'Last Notification: <b id=\"info-b\">' + locations[i][8] + '</b><br />'+
        'AQI: <b id=\"info-b-aqi\" style=\"color:' + locations[i][11] + '\">' + locations[i][9] + '</b><br />'+
        'Location: <b id=\"info-b\">' + locations[i][10] + '</b>.</p>'+
        '</div>'+
        '</div>';
        
        var icon_active = {
          url: '/images/' + locations[i][33],
          origin: new google.maps.Point(0, 0),
          labelOrigin: new google.maps.Point(11, 60)
        };
        
        var icon_passive = {
          url: '/images/' + locations[i][34],
          origin: new google.maps.Point(0, 0),
          labelOrigin: new google.maps.Point(11, 60)
        };
      ...
      else
      {
        ...
        var circle = new google.maps.Circle({
          ...
          fillOpacity: 0.35,
          map: map,
          center: new google.maps.LatLng(locations[i][12], locations[i][13]),
          radius: 500
        });
      }
...
// Renders the color code corresponding to the AQI value
function renderColorCodeFromAQI($aqi)
{
  // AQI values
  if ($aqi <= 50)
  {
  ...
  elseif ($aqi > 150 && $aqi <= 200)  // Moderate Breeze
  {
    $color_code = '#ff0000'; // Ademir: Unhealthy: rgb(255,0,0)
  }
  elseif ($aqi > 200 && $aqi <= 300)  // Fresh Breeze
  {

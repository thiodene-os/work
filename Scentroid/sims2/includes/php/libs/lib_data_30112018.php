...
function buildSensorsTable($company)
{
  ...
  $sensors_table_footer  = '<tr><td colspan="6"></td><td><span class="button button_add add_sensor">Add Sensor</span></td></tr></tbody></table>'
  ...
  if (@mysqli_num_rows($result) > 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
      ...
      $sensors_table_body .= '<tr><td>' . $row['sensor_name'] . '</td><td>' . $row['packet_id'] . '</td><td>' . $row['type']
    ...
    $final_sensor_table = $sensors_table_header . $sensors_table_body . $sensors_table_footer ;
  }
  // Close db
  db_close($dbc) ;

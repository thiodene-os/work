...
function buildAllSensorsTable($equipment, $chart_number)
{
	...
	if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
    {
      ...
      // Get the standard AQI for this equipment
      $query8 = "SELECT aqi_sensor_standard.id AS aqi_id, value
      ...
      $final_sensor_table .= "<tr chemical=\"" . $sensor_name . "\"><td title=\"" . $last_sample_time . "\">" . $sensor_name . "</td><td class=\"avg\">" . $last_sample_value . " " . $sensor_dataunit . "</td>"
                       . "<td class=\"aqi\">" . $aqi_display . "</td><td><span class=\"option selectable remove\" chart=\"" . $chart_number . "\" plot_number=\"" . $plot_number 
                       . "\" series=\"" . $plot_number . "\">PLOT<span class=\"check_plot\"><i class=\"fa fa-check fa-2x\"></i></span></span>"
                       . "<span class=\"option selectable add\" chart=\"" . $chart_number . "\" plot_number=\"" . $plot_number 
                       . "\" series=\"" . $plot_number . "\" sensor_name=\"" . $sensor_name . "\" axisyindex=\"" . $axisyindex 
                       . "\" axisytype=\"" . $axisytype . "\">PLOT</span></td>"
      ...
      // Get the 1/8/24H averages and the global AQI
      //$avg1 = 0; $avg8 = 0; $avg24 = 0; $aqi_std = 0;
      $query7 = "SELECT aqi_sensor_standard.id AS aqi_id, hour1_avg,  hour8_avg, hour24_avg, value
                 FROM aqi_sensor_standard
                WHERE sensor_id = " . $sensor_id ;
      $result7 = mysqli_query($dbc_local, $query7) or trigger_error("Query: $query7\n<br>MySQL Error: " . mysqli_error($dbc_local));
      ...
      $aqi_std = $row7[4] ;
      if ($aqi_std)
        $aqi_std_display = $aqi_std ;
      else
        $aqi_std_display = '-' ;
      
      $sensor_avg_array_js .= '["' . $sensor_name . '","' . $sensor_dataunit . '","' . $last_sample_value . '","' 
                                . $avg1_display . '","' . $avg8_display . '","' . $avg24_display
                                . '","' . $aqi_std_display . '","' . $aqi_std_display 
                                . '","' . $aqi_std_display . '","' . $aqi_std_display . '"]';
...
// Build the JS array that lists the averages and AQI values per sensors for one equipment
function buildSensorsAQIandAveragesJS($equipment)
{
	...
	if (@mysqli_num_rows($result) != 0) 
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
    {
      ...
      $sensor_avg_array_js .= '["' . $sensor_name . '","' . $last_sample_value . '","' 
                                . $avg1_display . '","' . $avg8_display . '","' . $avg24_display
                                . '","' . $aqi_std_display . '","' . $aqi_std_display 
                                . '","' . $aqi_std_display . '","' . $aqi_std_display
                                . '","' . $sensor_dataunit . '"]';
  ...
  return $sensor_avg_array_js ;
} // buildSensorsAQIandAveragesJS
...
//        text: \"$equipment_name\",
      ...
      toolTip:{
        ...
        borderThickness: '2',
        fontColor: 'white',
        content: 'Date: <strong>{x}</strong><br/>{name}: <strong>{y}</strong>'
      },
...
function get_all_equipment_averages($equipment) {
    ...
    $final_sensor_array[$final_sensor_iter][4] = '24 Hours';
    $final_sensor_array[$final_sensor_iter][5] = 'AQI';
    $final_sensor_array[$final_sensor_iter][6] = 'AQI';
    $final_sensor_array[$final_sensor_iter][7] = 'AQI';
    $final_sensor_array[$final_sensor_iter][8] = 'AQI';
    $final_sensor_array[$final_sensor_iter][9] = 'Data Unit';
    $final_sensor_iter++;
    ...
    if ($sensor_obj) {
        foreach ($sensor_obj as $value) {
            ...
            $final_sensor_array[$final_sensor_iter][4] = $value->value;
            $final_sensor_array[$final_sensor_iter][5] = rand(1, 1000) / 10;
            $final_sensor_array[$final_sensor_iter][6] = rand(1, 1000) / 10;
            $final_sensor_array[$final_sensor_iter][7] = rand(1, 1000) / 10;
            $final_sensor_array[$final_sensor_iter][8] = rand(1, 1000) / 10;
            $final_sensor_array[$final_sensor_iter][9] = $value->unit;
            $final_sensor_iter++;
        }
    }
    return $final_sensor_array;
